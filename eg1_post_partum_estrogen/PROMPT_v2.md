# Claude Code Prompt: pp_estrogen_v2 Research Digest
...
**Working directory:** `~/paperclip-test/pp_estrogen_v2/`

**You may read from:**
- `~/paperclip-test/pp_estrogen_v2/` (working directory — read and write)
...
**You may write to:**
- `~/paperclip-test/pp_estrogen_v2/` only

**You may NOT, under any circumstances:**
- Read, write, modify, or delete any file outside `~/paperclip-test/pp_estrogen_v2/` except the Paperclip documentation file
...
- Modify or delete the scaffold file (`pp_estrogen_research_digest_scaffold.md`) — treat it as a read-only template
...
Fill out the research digest scaffold at `~/paperclip-test/pp_estrogen_v2/pp_estrogen_research_digest_scaffold.md` for a clinician-facing Era brief on low-dose transdermal estrogen for postpartum mood stabilization.
...
**Save the filled version as `pp_estrogen_v2_filled.md` in the working directory. Preserve the original scaffold as a template.**

Run end-to-end. Do not pause for input.

## Reference materials

- **Scaffold to fill in:** `~/paperclip-test/pp_estrogen_v2/pp_estrogen_research_digest_scaffold.md`
...
- **Maintain a progress log** at `~/paperclip-test/pp_estrogen_v2/_progress.md`. Update at the start of each major step. Format: `[HH:MM] <phase>: <status>`.
- **Refer back to PROMPT.md as the source of truth** for what to do next. Do not improvise the workflow from memory.
- **Checkpoint after each completed sub-question.** Save the partial filled scaffold to `pp_estrogen_v2_filled.md` after SQ1 completes, after SQ2 completes, etc. Do not wait until the end.
...
1. **Broad scoping searches.** Run 3-4 broad parallel searches on the overarching question — not the sub-questions, the *overall question* of transdermal estrogen for postpartum mood stabilization in lactating patients. Example query directions (use your own phrasing):
   - "transdermal estradiol postpartum depression prevention treatment review"
   - "estrogen therapy postpartum mood disorders high-risk women"
   - "hormonal treatment postpartum depression breastfeeding lactation safety"
   - "perinatal mood disorders pharmacological intervention review"
...
2. **Identify anchor candidates** from the scoping results using multiple signals:
   ...
   - **Papers that span multiple sub-questions** (engage multiple aspects of the overarching question rather than addressing a single sub-question in isolation) — these are exactly the integrative papers most likely to be missed by sub-question-specific filters
...
**7. Checkpoint:** save updated `pp_estrogen_v2_filled.md` to disk before moving to the next sub-question.
...
- If the sub-question scope includes both general and specific framings (e.g., "transdermal estrogen for postpartum mood" AND "estrogen-class effects on perinatal mood broadly"), the filter must allow both.
...
**14. Final report.** Write to `~/paperclip-test/pp_estrogen_v2/_run_report.md`:
...
- **Distinguish direct from extrapolated evidence.** Especially for SQ3 (lactation) — combined oral contraceptive supply-suppression data is not the same as transdermal estradiol-only data. Surface the distinction in the table and synthesis.
- **Thin evidence is signal, not a search failure.** Do not broaden searches to pad tables.
- **DOI or PMID required for every row.**
- **Do not draft a brief or review article.** Structured findings only.

## Deliverables

- `~/paperclip-test/pp_estrogen_v2/pp_estrogen_v2_filled.md` — filled-in digest
- `~/paperclip-test/pp_estrogen_v2/pp_estrogen_research_digest_scaffold.md` — preserved as template (do not modify)
- `~/paperclip-test/pp_estrogen_v2/_anchor_candidates.md` — Phase 0 output
- `~/paperclip-test/pp_estrogen_v2/_progress.md` — timestamped step log
- `~/paperclip-test/pp_estrogen_v2/_run_report.md` — final summary report, including missing anchors section
